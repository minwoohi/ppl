...
public class DoActorWriteActionServlet extends HttpServlet {
	
	private static final long serialVersionUID = 1L;
    private ActorService actorService;
    
    public DoActorWriteActionServlet() {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (user.getAuthorizationId().equals(AuthConst.ADMIN_USER)
			...
			if( actorService.addActor(actorVO) ) {
				...
				response.sendRedirect("/ppl/actor/write");
			}
		}else {
